...
function updateIfNeeded() {
  ...
  lastUpdateCheckDay = NSUserDefaults.standardUserDefaults().stringForKey(manifestJSON.identifier + '.lastUpdateCheckDay')
...
function isTodayNewDay() {
  ...
  NSUserDefaults.standardUserDefaults().setObject_forKey(today, manifestJSON.identifier + '.lastUpdateCheckDay')
...
function checkPluginUpdate() {
  ...
  try {
    var response = getJSONFromURL('https://raw.githubusercontent.com/DWilliames/PDF-export-sketch-plugin/master/PDF%20Export.sketchplugin/Contents/Sketch/manifest.json')
